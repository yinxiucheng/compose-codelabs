...
import com.yxc.customercomposeview.calander.listener.OnClickDisableDateListener;
import com.yxc.customercomposeview.calander.listener.OnYearMonthChangedListener;
...
public abstract class BaseCalendar extends ViewPager {
    ...
    protected BaseCalendarPainter mCalendarPainter;


    public BaseCalendar(@NonNull Context context, @Nullable AttributeSet attributeSet) {
        super(context, attributeSet);
        this.attrs = CalendarAttrsUtil.getAttrs(context, attributeSet);
        this.mCalendarPainter = new InnerPainter(attrs);
        init(context);
    }


    public BaseCalendar(Context context, CalendarAttrs attrs, BaseCalendarPainter calendarPainter) {
        super(context);
        this.attrs = attrs;
    ...
    public void jumpDate(LocalDate localDate, boolean isDraw, boolean isClick) {
        localDate = getSelectDate(localDate);
        int num = getTwoDateCount(mSelectDate, localDate, attrs.firstDayOfWeek);
        onClickDate(localDate, num, isClick);
    }

    //跳转
    public void jumpDate(LocalDate localDate, boolean isDraw) {
        localDate = getSelectDate(localDate);
        int num = getTwoDateCount(mSelectDate, localDate, attrs.firstDayOfWeek);
        onClickDate(localDate, num, true);
    }

    protected void onClickDate(LocalDate localDate, int indexOffset) {
        onClickDate(localDate, indexOffset, true);
    }

    //点击处理
    protected void onClickDate(LocalDate localDate, int indexOffset, boolean isClick) {
    ...
    private void callBack(boolean isDraw, boolean isClick) {
        if (!mSelectDate.equals(callBackDate)) {
            //选中回调 ,绘制了才会回到
            if (isDraw) {
                onSelcetDate(mSelectDate, isClick);
    ...
    private LocalDate getSelectDate(LocalDate localDate) {
        if (localDate.isBefore(startDate)) {
            ...
            return endDate;
        } else if (TimeDateUtil.isFuture(localDate)){
    ...
    //回去viewpager的adapter
    protected abstract BaseCalendarAdapter getCalendarAdapter(Context context, CalendarAttrs attrs, LocalDate initializeDate);
    ...
    //日历上面选中的日期，有选中圈的才会回调
    protected abstract void onSelcetDate(LocalDate localDate, boolean isClick);
    ...
    //设置绘制类
    public void setCalendarPainter(BaseCalendarPainter calendarPainter) {
        this.mCalendarPainter = calendarPainter;
       // notifyAllView();
    }
