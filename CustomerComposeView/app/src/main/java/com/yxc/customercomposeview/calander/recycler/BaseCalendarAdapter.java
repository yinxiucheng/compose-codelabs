...
public abstract class BaseCalendarAdapter extends PagerAdapter {
    ...
    protected CalendarAttrs mAttrs;//属性参数
    protected LocalDate mInitializeDate;//日期初始化，默认是当天


    public BaseCalendarAdapter(Context context, CalendarAttrs attrs, LocalDate initializeDate) {
        this.mContext = context;
        this.mAttrs = attrs;
        this.mInitializeDate = initializeDate;
        LocalDate startDate = new LocalDate(attrs.startDateString);
        //未来的日期这里不显示了，没有数据
        this.mCount = getIntervalCount(startDate, mInitializeDate, attrs.firstDayOfWeek) + 1;
